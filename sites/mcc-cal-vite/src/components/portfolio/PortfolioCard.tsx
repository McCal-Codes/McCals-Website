...
const PortfolioCard: FC<PortfolioCardProps> = ({
  ...
  return (
    // A plain container. It used to be role="button" with tabIndex 0 while
    // containing the copy-link button, which axe reports as
    // nested-interactive: a control must not hold focusable descendants, and a
    // screen reader user met two overlapping controls with no way to tell them
    // apart. The action lives on the stretched button at the end of this card.
    <article ref={cardRef} className={portfolioStyles.pfCard} id={group.id}>
      {/* Photo count badge */}
      ...
      <ProtectedPortfolioImage>
        <OptimizedImage
          ...
          sizes={
            imageSizes ?? '(max-width: 600px) calc(100vw - 40px), (max-width: 900px) 50vw, 33vw'
          }
          width={400}
          height={300}
          draggable={false}
      ...
      {/*
        Last child so it stacks over the image and the overlay. The copy button
        sits above it at a higher z-index and stays clickable. This carries the
        card's accessible name, which the overlay above cannot: that is
        aria-hidden, because it repeats what this label already says.
      */}
      <button
        type="button"
        className={portfolioStyles.pfCardOpen}
