...
- `src/widgets/cdn/jsdelivr-loader.js` — small runtime that finds `.mccal-widget` (or `[data-cdn-widget]`) containers and loads HTML from jsDelivr. If the CDN fetch fails it can fall back to a provided local path.
...
3. Create the jsDelivr URL for that exact file and tag and test it using `widget-snippet.html` (recommended: set `data-src` to the full URL).
4. When validated, update your Squarespace Code Block snippet to point `data-src` at the pinned jsDelivr URL, and include the loader from jsDelivr.
5. If you need an immediate revert, repoint `data-src` back to the previous tag URL in the page markup (no site-side code changes needed beyond that).
...
- Do not remove or rename existing versioned widget files in `src/widgets/<widget>/versions/` — the loader's fallback option may rely on those files for preview/testing.

Common container attributes

- `data-src`: full jsDelivr URL (recommended)
- `data-dev-src`: optional dev override URL (used automatically on localhost, or with `?mccalWidgetDev=1`)
- `data-fallback`: optional URL/path used if the primary fetch fails
- `data-mode`: `inline` (default) or `iframe`
- `data-replace`: when `true`, clears the container before injecting widget HTML (useful for replacing placeholders)
- `data-debug`: when `true`, logs more detailed errors to the console (also available via `?mccalWidgetDebug=1`)
