...
(function () {
  ...
  function buildJsDelivrUrl(repo, path, version) {
    ...
    let base = "https://cdn.jsdelivr.net/gh/" + repo + "/";
  ...
  function runInlineInjection(container, html, baseUrl) {
    ...
    if (baseUrl) {
      const elems = tpl.content.querySelectorAll(
        "link[href], img[src], script[src], a[href]"
      );
  ...
  function fetchAndInject(container, url, fallback) {
    let baseForRelative = null;
    try {
      baseForRelative =
        new URL(url).origin + new URL(url).pathname.replace(/\/[^/]*$/, "/");
    } catch (e) {
      baseForRelative = null;
    }
    return fetch(url, { mode: "cors" })
      ...
      .catch(function (err) {
        // try fallback
        if (fallback) {
          return fetch(fallback, { mode: "same-origin" })
            .then(function (r) {
  ...
  function initOne(container) {
    ...
    const directSrc = container.getAttribute("data-src");
    const repo = container.getAttribute("data-repo");
    const path = container.getAttribute("data-path");
    const version = container.getAttribute("data-version");
    const fallback = container.getAttribute("data-fallback");
    const mode = (
      container.getAttribute("data-mode") || "inline"
    ).toLowerCase();

    const url =
      directSrc ||
      (repo && path ? buildJsDelivrUrl(repo, path, version) : null);
    if (!url && !fallback) {
      container.innerHTML =
        '<div style="padding:18px;color:#666">Widget configuration missing (data-src or data-repo+data-path required).</div>';
    ...
    return fetchAndInject(container, url, fallback).catch(function (err) {
      console.warn(
        "McCalJsDelivrWidgetLoader: failed to load widget - jsdelivr-loader.js:116",
        err
      );
      container.innerHTML =
        '<div style="padding:18px;color:#666">Widget failed to load.</div>';
    });
  }
  ...
  const API = {
    loadInto: function (selectorOrNode, opts) {
      ...
      if (selectorOrNode && selectorOrNode.nodeType)
        return initOne(selectorOrNode);
