...
if (typeof window === "undefined" || typeof document === "undefined") {
  ...
  console.error("ERROR:  must be run in a browser (DOM environment). - journalism-widget-v5.1.js:4");
...
(function(){
  ...
  async function fetchManifest(force){if(force)BUST=Date.now(); let cached=!force&&getCache(); if(cached){log('cache hit'); return cached;} metrics.api++; const url=manifestUrl(); log('fetch',url); const txt=await fetch(url,{cache:'no-store'}).then(r=>{if(!r.ok) throw new Error('HTTP '+r.status); return r.text()}); const json=JSON.parse(txt); if(!json || !Array.isArray(json.events)) throw new Error('Bad manifest shape'); manifestGenerated=json.generated||''; setCache(json); return json}
  ...
  function buildCard(ev){
    ...
    const open=()=>{console.log('[DEBUG] openLB called for - journalism-widget-v5.1.js:58', ev); openLB(ev);};
    card.addEventListener('click',open);
    card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}});
    console.log('[DEBUG] buildCard created card for - journalism-widget-v5.1.js:61', ev.eventName, 'with click listener:', !!card.onclick);
  ...
  function openLB(ev){
    console.log('[DEBUG] openLB triggered for - journalism-widget-v5.1.js:67', ev.eventName, 'images:', ev.images);
    const lb=sel('#journalismLightbox'), gal=sel('#jlGallery');
    if(!lb||!gal){console.log('[DEBUG] Lightbox or gallery element missing! - journalism-widget-v5.1.js:69'); return;}
    gal.innerHTML='<div class="jl-hint">Scroll Up/Down</div>';
    (ev.images||[]).forEach((it,i)=>{
      const im=new Image(); im.loading='lazy'; im.alt=ev.eventName+' - '+(i+1); const url=resolveImagePath(ev,it); im.classList.add('loading'); im.onload=()=>{im.classList.remove('loading'); im.classList.add('loaded')}; im.onerror=()=>{im.classList.remove('loading'); im.classList.add('error'); im.alt='Image failed'}; im.src=url; gal.appendChild(im);
    });
    ...
    setTimeout(()=>{const h=gal.querySelector('.jl-hint'); if(h) h.classList.add('fade')},3000)
  }
  ...
  sel('#dbgTests - journalism-widget-v5.1.js:99')&&sel('#dbgTests').addEventListener('click',()=>{console.log('[v5.1 tests] start'); console.log('cards',all('.journalism-card').length);});
