...
- For Squarespace code-block drops, run `npm run site-widgets:build` so the stylesheet is automatically inlined inside every widget version.
...
<link
  rel="stylesheet"
  href="https://cdn.jsdelivr.net/gh/McCal-Codes/McCals-Website@site-widgets@0.1.0/src/widgets/_shared/site-widgets.css"
  integrity=""
...
Inline pipeline (repo default)

- Run `npm run site-widgets:build` whenever shared tokens change; the script writes `dist/site-widgets/site-widgets.{css,min.css}` and injects the minified CSS block into every widget version between:
...
- Place `<!-- site-widgets:inline:skip -->` near the top of a widget file to opt out (reserved for experiments/WIP). CI (`npm run validate:widgets`) enforces that all other versions include the block.
- Editing the inline block by hand is discouraged—update `site-widgets.css` and re-run the build script so every widget stays in sync.
