...
- **NEW**: Anchored links — Each event card has a deep-linkable anchor (#event-name). Hover to reveal chain icon, click to copy URL.
- **NEW**: Hash navigation — Visiting a URL with `#event-name` auto-scrolls to the card and opens its lightbox.
...
- **PERF**: Extended cache TTL to 30 minutes and enabled browser caching for faster subsequent loads.
- **RETAINED**: All v5.2 features (glass buttons, one image per album, constrained shuffle).
