...
### A Script Wrote "AI session" Into the Changelog

- `scripts/utils/ai-finalize-session.js`, run by `npm run ai:finalize`, inserted `- <date>: AI session ...` into `CHANGELOG.md`. It does not commit, so a person committed it, but the text was attribution written into the repository by a tool. It now writes a neutral `### Repository maintenance` fragment instead, which also matches the new convention. Its other output path used an em-dash and now uses a colon.
- The check in `no-ai-attribution.yml` did **not** catch that line, and still would not. It targets the machine-inserted attribution forms: an AI `Co-Authored-By`, a vendor no-reply address, a "generated with" advertisement, a robot marker. It is not a general detector for the word AI, deliberately, because prose that discusses AI is allowed. The script was found by reading, not by the check.
- `copilot-instructions-guardian.yml` also required a `CHANGELOG.md` edit and had to learn about fragments too, which its own run on this pull request is what surfaced. Those are now the only two places that gate on a changelog.
