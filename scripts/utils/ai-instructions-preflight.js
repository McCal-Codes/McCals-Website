...
const REQUIRED_DOCS = [
  ...
  {
    key: 'codex',
    file: path.join(ROOT, '.github', 'codex-instructions.md'),
...
function summarizeDoc(key, title, content) {
  ...
  sections.versioning = tryHeading(
    ['versioning', 'version standardization', 'version standardization complete (x.x.0 format)'],
    limitShort,
  );

  const norms = [
    ...tryHeading(
      ['key rules', 'safe-change checklist for agents', 'safe-change checklist'],
      isShort ? 4 : 6,
    ),
    ...tryHeading(['codex summary', 'agent responsibilities'], isShort ? 2 : 3),
  ].slice(0, isShort ? 6 : 8);

  const checklist = tryHeading(
    [
      'pre-call checklist',
...
function main() {
  ...
  const repoNorms = dedupe(outputs.flatMap((o) => (o.summary && o.summary.norms) || [])).slice(
    0,
    isShort ? 6 : 10,
  );
