...
async function downloadLogo(logoKey, config) {
  ...
  console.log(`\n🔍 Downloading logo for: ${name}`);
  
  // Try each source URL
  for (let i = 0; i < sources.length; i++) {
    const url = sources[i];
    console.log(`  Trying source ${i + 1}/${sources.length}: ${url.substring(0, 60)}...`);
    ...
    try {
      ...
      if (isValid) {
        ...
        console.log(`  ❌ Downloaded file is invalid, trying next source...`);
  ...
  console.log(`  📝 Creating SVG placeholder for ${name}`);
  const svgPath = path.join(LOGOS_DIR, `${logoKey}-logo.svg`);
  const svg = createPlaceholderLogo(logoKey, name);
  fs.writeFileSync(svgPath, svg);
  console.log(`  ✅ Created placeholder: ${logoKey}-logo.svg`);
  
  return false;
}

async function main() {
  console.log('🚀 McCal Media Logo Downloader');
  console.log('===============================\n');
  ...
  console.log(`  📝 SVG placeholders created: ${results.placeholder}`);
  console.log(`  ❌ Failed: ${results.failed}`);
  console.log(`  📁 Files saved to: ${LOGOS_DIR}`);
  
  // Update the about page to use local files instead of GitHub URLs
  console.log('\n🔄 Next steps:');
  console.log('  1. Run this script: node scripts/logo-downloader.js');
  console.log('  2. Check the downloaded logos in assets/images/logos/');
  console.log('  3. Update the about page to use local file paths');
