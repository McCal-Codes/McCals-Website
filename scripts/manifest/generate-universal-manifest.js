...
const {
  ...
  createFallbackDate,
  MONTHS,
} = require('../utils/shared-date-parsing.js');
...
function shouldSkipFolder(name) {
  ...
  return (
    ...
    normalized === 'thumbs' ||
    // Selected/ holds the individual photographs curated for /featured-work by
    // scripts/manifest/featured-curation.json. It is not a gallery: it has no
    // route, no album structure and no tags.json, and the frames in it are shown
    // one at a time with their own captions. Left in, this loop invented a sixth
    // portfolio type called "Selected Photography" with one item in it, and
    // seo-auto-update.yml would then commit that churn to main after every merge,
    // costing a second production build of every project for data nothing reads.
    normalized === 'selected'
  );
}
...
async function processPortfolioItem(portfolioType, itemName, itemPath) {
  ...
  try {
    ...
    for (const subfolder of subfolders) {
      ...
      const folderImages = dedupeImageEntries(
      ...
      const dateDisplay = override
        ? override.dateDisplay
        : getDateDisplayFromFolder(subfolder.name) || `${date.monthName} ${date.year}`;
      date.display = dateDisplay;
...
async function processPortfolioType(portfolioType, portfolioPath) {
  ...
  try {
    ...
    if (directImages.length > 0) {
      ...
      const dateDisplay = detectedDate
...
async function generateUniversalManifest() {
  ...
  try {
    ...
    log(
      `Found ${validPortfolioTypes.length} portfolio types:`,
      validPortfolioTypes.map((t) => t.name),
    );
    ...
    const written = await writeManifestIfChanged(MANIFEST_OUTPUT, universalManifest);
    try {
      await notify('universal', { path: MANIFEST_OUTPUT, written });
    } catch (err) {
    ...
    success(
      `Processed ${allItems.length} portfolio items with ${universalManifest.totalImages} total images`,
    );
    ...
    Object.entries(portfolioSummary).forEach(([type, stats]) => {
      console.log(
        `• ${type}: ${stats.count} items, ${stats.totalImages} images - generate-universal-manifest.js:407`,
      );
    });
    ...
    categories.forEach((category) => {
      ...
      console.log(
        `• ${category}: ${categoryItems.length} items - generate-universal-manifest.js:413`,
      );
    });
  } catch (err) {
