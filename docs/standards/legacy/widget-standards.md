...
The workspace auto-checker (see `scripts/admin/welcome.js`, with a thin wrapper kept at `scripts/welcome.js` for backwards compatibility) now supports:
...
### 1b. **Shared Widget Stylesheet Pipeline (2025-12 update)**

To keep widgets self-contained while still benefiting from site-wide tokens, every active widget version must include the shared stylesheet block generated by `npm run site-widgets:build`.

**Workflow**

1. Edit `src/widgets/_shared/site-widgets.css` when updating shared tokens/utilities.
2. Run `npm run site-widgets:build` (optionally `--dry-run` or `--no-inline`).

- The script writes prettified/minified outputs to `dist/site-widgets/`.
- It also injects the CSS into every `src/widgets/**/versions/*.html` file using the markers below.

3. Commit the widget version changes (the shared block lives directly in each HTML file).
4. `npm run validate:widgets` now fails if a version file is missing the inline block.

**Inline Marker Format**

```html
<!-- site-widgets:inline:start -->
<style data-site-widgets-inline>
  /* contents of site-widgets.min.css */
</style>
<!-- site-widgets:inline:end -->
```

Placeholders are inserted automatically at the top of each widget file (before the first `<style>` tag) so widget-specific `:root` declarations continue to override the shared defaults. To opt out for a particular file, add `<!-- site-widgets:inline:skip -->` near the top; `validate:widgets` recognizes this flag.

> **Note:** Do not edit the generated inline block directly. Update `site-widgets.css` and rerun `npm run site-widgets:build` to regenerate all files.
