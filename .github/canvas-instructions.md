## Canvas instructions - McCal Media edit etiquette in Canvas

- Don't modify `dist/**` (build output) or generated `manifest.json` files.
- When changing manifest fields, update: generator(s), CI (if needed), docs, and any consumers (for example `src/site/app.js` and relevant widgets).
...
- **Add entries to the Recent updates section** when you make significant changes to document what was learned or improved.
...
- Update a widget: edit the latest `versions/vN.M.html`; if behavior changes, bump minor version (for example `v1.0 -> v1.1`) and keep the old file.
...
- Generator: `scripts/enhanced-manifest-generator.js` parses dates from filenames (`YYMMDD`, `YYYMMDD`, `DD-MM-YY` edge cases) or EXIF; writes per-folder `manifest.json` and a concert `processing-summary.json`.
- Key outputs:
...
- GitHub Actions regenerate and commit manifests on pushes to `src/images/Portfolios/**` for Concert, Events, and Journalism.
- PR guard: `.github/workflows/copilot-instructions-guardian.yml` reminds contributors to keep AI instructions and the changelog updated.
...
- `scripts/enhanced-manifest-generator.js` - manifest shape and logic
...
- 2025-10-24T23:14:01.899Z - Successfully created Portrait Portfolio v1.0 widget with vertical composition focus, 3:4 aspect ratios, enhanced detail viewing, performance optimizations, and SEO features. Added to available widgets list and created a sample manifest.
- 2025-10-24T22:21:24.273Z - Successfully implemented podcast widget v1.9.5 with auto-hydrating RSS episodes. Added Ep 9 fallback data, live RSS caching, and updated show branding. Created a test page that works properly. The widget now auto-populates new episodes without manual updates.
...
- 2025-10-06T23:40:47.794Z - Created a comprehensive SEO starter guide tailored for McCal Media's Squarespace implementation, covering site structure, titles/meta descriptions, image optimization, structured data, internal linking, and technical hygiene. Added it to `docs/standards/` and updated the main `README.md` and `CHANGELOG.md`.
- 2025-10-06T21:59:34.422Z - Fixed concert portfolio widget v4.6 image loading and layout issues. Updated CSS to use a responsive column-width layout with overlay info styling, matching journalism widget patterns. Images now load properly in a masonry-style grid with smooth transitions and loading animations.
- 2025-10-06T21:54:49.567Z - Created a performance-optimized concert portfolio widget v4.6 with critical CSS inlining, modern JavaScript patterns, and reduced main-thread blocking. Addresses PageSpeed issues: render-blocking resources, unused CSS/JS, and long tasks.
- 2025-10-06T21:54:42.902Z - Created a performance-optimized concert portfolio widget v4.6 with critical CSS inlining, modern JavaScript patterns, and reduced main-thread blocking. Addresses PageSpeed issues: render-blocking resources, unused CSS/JS, and long tasks. Added resource hints and lazy-loaded features.
- 2025-10-06T21:42:37.717Z - Fixed structured data detection issues in concert portfolio widget v4.5. Updated `addBasicStructuredData` to calculate total images from manifest bands, generate absolute image URLs, and add comprehensive metadata. Added debug functionality to check structured data locally. Structured data now includes proper Schema.org `ImageGallery` markup with image URLs, author info, and SEO metadata.
...
- 2025-10-06T21:19:09.070Z - Successfully tested and fixed VS Code tasks for Copilot AI workflow. Fixed PowerShell quoting issues in the widget validation task, created a proper Node.js validation script, and verified all core tasks work correctly.
...
- 2025-10-04T00:00:00.002Z - Added agent responsibilities and widget preview workflow guidance: agents must read/update instructions and use the user-driven preview workflow instead of external hosting.
- 2025-10-03T09:15:17.087Z - Featured Portfolio Widget v1.5 complete: enhanced journalism titles, ultra-minimal scrollbars (4px, 0.15 opacity), improved masonry spacing (16px gaps), randomized cover images with Fisher-Yates shuffle, minimal gray accents (`#888888`), production-ready deployment with a 15-item limit, scrollable lightbox, and comprehensive changelog documentation.
- 2025-10-03T09:15:11.679Z - Session complete: minor updates.
- 2025-10-03T07:08:00.405Z - Fixed the featured portfolio widget by creating `generate-featured-manifest.js` and updated the widget to v1.2 with better debugging.
