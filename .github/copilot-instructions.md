...
- The technical portfolio is **not in this repository**. It lives in `McCal-Codes/mccal-codes.github.io` and publishes to GitHub Pages. Software and product work goes there, not on the photography site.
...
- 2026-09-07: `sites/mcc-cal-dev` was extracted to `McCal-Codes/mccal-codes.github.io` and now deploys to GitHub Pages; its Vercel project and `dev.mcc-cal.com` are retired. The 2026-08-06 entry below is kept as history, but its claim that `/terranova` and `/roadmap` redirect to the dev site was never true: no such redirects existed in either `vercel.json`.
