TODO Tree Extension Compatibility (2025-10-09) — UPDATED

Purpose

This file defines agent behavior and workspace guardrails and enforces Todo Tree compatibility for VS Code.

Key rules

- Use explicit TODO:/FIXME:/BUG: tags for code-tracked items (start of line). Use markdown checklists (- [ ]) for planning only.
- When adding a TODO in code, also add an entry in updates/todo.md or updates/done.md for traceability.

Codex summary

- Widgets: self-contained HTML in src/widgets/<name>/versions/ (inline CSS/JS, no external runtime deps).
- Manifests: generated by scripts, do not edit dist/ or aggregated manifests by hand.
- Versioning: tag per-widget with format widget-name@MAJOR.MINOR.PATCH.
...
- [ ] TODO: Add CI job for pre-publish integrity check
