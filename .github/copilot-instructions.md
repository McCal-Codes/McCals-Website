...
- 2026-09-12: `/featured-work` is a sequence of individual photographs, not albums. `scripts/manifest/featured-curation.json` is the only input, and the frames it lists are the frames that appear, in that order. `generate-featured-manifest.js` emits `frames[]` carrying per-frame `caption`, `alt`, `width`, `height` and `album`, replacing the old `items[]` of albums, and it fails the build on a curated path that does not resolve rather than emitting a url that 404s. Read `frames`, not `items`.
