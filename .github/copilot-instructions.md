...
- `sites/mcc-cal-dev/` -> technical portfolio; content is a typed schema in `src/content/`. `github.json` and `sites.json` are generated by the sync scripts, not hand-edited. Anything GitHub can measure is deliberately absent from the hand-written types, so it cannot be written by hand and cannot drift
...
- 2026-08-24: `sites/mcc-cal-dev` gained a websites section for client work, fed by `scripts/sync-sites.js`. A site is not a `Project`: a product is proved by its repository, a website by being live, and hosted-platform sites have no repository at all. Previews are Playwright screenshots stored under `public/site-previews/`, never hotlinked, because the CSP is `connect-src 'self'`.
- 2026-08-24: `scripts/validate-content.js` runs as part of that app's build and fails it when a slug in a `.ts` content file has no matching record in a synced `.json`. TypeScript cannot catch this: a mistyped `repoSlug` type-checks and silently renders a missing link.
- 2026-08-24: Do not set `cleanUrls` in `sites/mcc-cal-dev/vercel.json`. It 308-redirects `/index.html` to `/`, which is where the SPA fallback rewrite points, so every route but `/` returns 404. The photography site carries the same rewrite and is unaffected because it builds from the repo root rather than a Root Directory.
- 2026-08-24: `<ScrollRestoration>` was removed from that app. It raced the hash-scroll hook and made `/#index` land inconsistently. `src/lib/useHashScroll.ts` owns scroll position; note that `focus()` aborts an in-flight smooth scroll, so focus is moved before scrolling, not after.
