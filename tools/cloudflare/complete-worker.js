...
const MANIFEST_CONFIG = {
  concert: { path: 'src/images/Portfolios/Concert/concert-manifest.json' },
  events: { path: 'src/images/Portfolios/Events/events-manifest.json' },
  journalism: { path: 'src/images/Portfolios/Journalism/journalism-manifest.json' },
  nature: { path: 'src/images/Portfolios/Nature/nature-manifest.json' },
  portrait: { path: 'src/images/Portfolios/Portrait/portrait-manifest.json' },
  featured: { path: 'src/images/Portfolios/featured-manifest.json' },
  portfolio: { path: 'src/images/Portfolios/portfolio-manifest.json' },
};
...
router.get('/api/v1/blog', () => {
  return jsonResponse({
    ...
    endpoints: ['/api/v1/blog/auth/login', '/api/v1/blog/posts', '/api/v1/blog/posts (POST)'],
  });
});
...
router.get('/api/v1/manifests/:type', async (request, env) => {
  ...
  if (!MANIFEST_TYPES.includes(type)) {
    return jsonResponse(
      { error: 'invalid_type', message: 'Unknown manifest type' },
      { status: 404 },
    );
  }

  const manifestConfig = MANIFEST_CONFIG[type];
  const fallbackBase = 'https://raw.githubusercontent.com/McCal-Codes/McCals-Website/main';
  const baseUrl = (env?.MANIFEST_BASE_URL || fallbackBase).replace(/\/+$/, '');
  const manifestUrl = `${baseUrl}/${manifestConfig.path}`;
  ...
  try {
    ...
    return jsonResponse({ error: 'fetch_failed', message: error.message }, { status: 503 });
  }
});
...
router.post('/api/v1/blog/auth/login', async (request, env) => {
  try {
    ...
    return jsonResponse({ error: 'invalid_request', message: error.message }, { status: 400 });
  }
});
...
router.get('/api/v1/blog/posts', async (request, env) => {
  ...
  try {
    ...
    return jsonResponse({ error: 'fetch_failed', message: error.message }, { status: 500 });
  }
});
...
router.post('/api/v1/blog/posts', async (request, env) => {
  ...
  try {
    ...
    return jsonResponse({ error: 'create_failed', message: error.message }, { status: 500 });
  }
});
...
// 404 handler
router.all('*', () => {
  return jsonResponse({ error: 'not_found', message: 'Endpoint not found' }, { status: 404 });
});
